...
class SuAdminMenu extends CMenu
{
	 ...
	 */
	protected function renderMenuRecursive($items, $isSubmenu=false)
	{
		$count=0;
		$n=count($items);
		foreach($items as $item)
		{
			...
			if(isset($item['items']) && count($item['items']))
			{
				$submenuHtmlAttributes = $this->submenuHtmlOptions;
				if ($isSubmenu){
					$submenuHtmlAttributes['class'] .= ' sub-menu';
				}
				echo "\n".CHtml::openTag('ul',isset($item['submenuOptions']) ? $item['submenuOptions'] : $submenuHtmlAttributes)."\n";
